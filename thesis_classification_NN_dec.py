...
path = "./thesis_models"
model_name = "/decreasing_3103_3212_norm26k_declr"
...
model = keras.Sequential([keras.layers.InputLayer(input_shape = (16,32,2,2)),
                 	  keras.layers.Conv3D(filters = 64, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),
                          #  keras.layers.Dropout(0.3),				
			#keras.layers.MaxPool3D(pool_size=(2,1,2),padding="same"), 
			keras.layers.LayerNormalization(),

			 keras.layers.Conv3D(filters = 32, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),
 			    keras.layers.Dropout(0.3),
			   keras.layers.MaxPool3D(pool_size=(2,1,2),padding="same"),
			keras.layers.LayerNormalization(),

			   keras.layers.Conv3D(filters = 16, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),
 			    keras.layers.Dropout(0.3),
			   keras.layers.MaxPool3D(pool_size=(2,1,2),padding="same"),
			keras.layers.LayerNormalization(),

			  keras.layers.Conv3D(filters = 8, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),#,kernel_initializer='he_uniform'),

			   keras.layers.Conv3D(filters = 4, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),#,kernel_initializer='he_uniform'),                        
                           keras.layers.Conv3D(filters = 2, kernel_size = (3,3,2),padding="same",activation="relu",kernel_initializer='glorot_normal'),
...
model.compile(loss='binary_crossentropy',
                optimizer = keras.optimizers.Adam(0.001,epsilon=0.0001),
